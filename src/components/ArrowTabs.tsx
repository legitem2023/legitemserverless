  ...
  return (
    <div className="bg-[#ff9999] flex w-full gap-1">
      {stages.map((stage, index) => {
        let clipPath = "polygon(12% 0%, 88% 0%, 100% 50%, 88% 100%, 12% 100%, 0% 50%)"; // Default: Gitnang tabs (arrow both sides)

        if (index === 0) {
          clipPath = "polygon(0% 0%, 100% 0%, 88% 50%, 100% 100%, 0% 100%)"; // Unang tab (flat left, arrow right)
        } else if (index === stages.length - 1) {
          clipPath = "polygon(0% 0%, 88% 0%, 100% 50%, 88% 100%, 0% 100%, 12% 50%)"; // Huling tab (flat right, baliktad na arrow left)
        }
